...
try{
 ...
 if($_POST){
  ...
  if($garson_id!="" && $garson_ad!=""){
    $sonuc = $conn->exec("UPDATE urun SET  urun_ad ='$garson_ad' , katag_id ='$katag_id' , altkategori_id ='$markas_id'   WHERE urun_id='$garson_id'");
  }else{
...
<div class="table-responsive">
  ...
  <tr ><th style='text-align:center;'>Ürün adi</th>
  <th style='text-align:center;'>Kategori adi</th>
  <th style='text-align:center;'>Alt Kategori adi</th>
...
try{
 ...
 $query = $conn->query("SELECT * FROM urun inner join kategori on (kat_id=katag_id) inner join altkategori on(altkategori.altkategori_id=urun.altkategori_id)");

 while ($row = $query->fetch()) {
  echo "<tr ><td style='text-align:center;'>".$row['urun_ad']."</td>";
  echo "<td style='text-align:center;'>".$row['kat_ad']."</td>";
  echo "<td style='text-align:center;'>".$row['altkategori_ad']."</td>";
    echo "<td style='text-align:center;'> <button class='urunduzen' id=".$row['urun_id'].">Düzenle</button>";
}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Ürün duzenle</h5>
  ...
  <input type="hidden" value="" id="garson_id" name="id">
    ...
    <label for="tur">Alt Kategori adı</label>
