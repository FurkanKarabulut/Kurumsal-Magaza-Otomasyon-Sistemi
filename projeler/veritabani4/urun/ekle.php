...
try{
 ...
 if($gad!="" ){
$query=$conn->exec("insert into urun (urun_ad,katag_id,altkategori_id) VALUES('$gad','$gad1','$gad2')");
if($query)
...
?>
  ...
  <input type="hidden" value="" id="garson_id" name="id">
    ...
    <input type="text" id="fname" name="gad"  placeholder="Ürün adı">
...
while ($row = $query->fetch()) {
  echo "<option value=".$row['kat_id']." selected>".$row['kat_ad']."</option>";
   ...
   ?>
    ...
    <label for="marka">Alt Kategori </label>
...
while ($row = $query->fetch()) {
  echo "<option value=".$row['altkategori_id']." selected>".$row['altkategori_ad']."</option>";
}
   ?>
    </select>
